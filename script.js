...
const profileUrl = `https://fylebackend.onrender.com/user/${userName}`;
const reposUrl = `https://fylebackend.onrender.com/repo?page=1&limit=10`;
...
async function handleLimitChange(n) {
    ...
    const reposLimitUrl = `https://fylebackend.onrender.com/repo?page=1&limit=${currLimit}`;
...
async function handleNext() {
    toggleLoader();
    currPage++;
    const reposLimitUrl = `https://fylebackend.onrender.com/repo?page=next&limit=${currLimit}`;
    const data = await getData(reposLimitUrl);
    showRepositories(data.data, data.page);
    toggleLoader();
};
...
async function handlePrev() {
    toggleLoader();
    currPage -= 1;
    const reposLimitUrl = `https://fylebackend.onrender.com/repo?page=prev&limit=${currLimit}`;
    const data = await getData(reposLimitUrl);
    showRepositories(data.data, data.page);
    toggleLoader();
}
...
async function handleSearch(search) {
    toggleLoader();
    query=search;
    const searchUrl = `https://fylebackend.onrender.com/search/${query}`;
